...
export default function ReturnBookPage() {
  ...
  useEffect(() => {
    ...
    const fetchBorrowedBooks = async () => {
      try {
        ...
        const parsedData = data.map((book: BorrowedBook) => ({
          ...book,
          due_date: new Date(book.due_date),
        }))
        setBorrowedBooks(parsedData)
        setLoading(false)
      } catch (error) {
        console.error(error)
      }
  ...
  const handleExtendDueDate = (book: BorrowedBook, days: number) => {
    const updatedBooks = borrowedBooks.map((b) =>
      b._id === book._id ? { ...b, due_date: addDays(b.due_date, days) } : b
    )
              ...
              {borrowedBooks.map((book) => (
                <Card key={book._id} className="flex flex-col overflow-hidden">
                  ...
                  <CardFooter className="p-4 pt-0 flex justify-between">
                    <Button
                      disabled={!!returnRequests.find((r) => r.book === book.book._id)}
                      variant="outline"
                      onClick={() => handleReturnBook(book.book._id, book.due_date)}
                    >
                      <ArrowLeftCircle className="h-4 w-4 mr-2" />
                      {returnRequests.find((r) => r.book === book.book._id) ? "Requested" : "Return"}
                    </Button>
                    <Dialog>
